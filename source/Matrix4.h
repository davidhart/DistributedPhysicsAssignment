...
public:
	...
	void column(unsigned int column, const Vector4f& value);
	const Vector4f& column(unsigned int column) const;

	void row(unsigned int row, const Vector4f& value);
	Vector4f row(unsigned int row) const;

	void Transform(Vector4f& inout) const;

	static void Identity(Matrix4& out);
	static void Translation(Matrix4& out, const Vector3f& translation);
	static void PerspectiveFov(Matrix4& out, float fov, float aspect, float nearclip, float farclip);
	static void LookAt(Matrix4& out, const Vector3f& eye, const Vector3f& at, const Vector3f& up);
	static void Scale(Matrix4& out, float scale);
	static void Scale(Matrix4& out, const Vector3f& scale);
	static void RotationAxis(Matrix4& out, const Vector3f& axis, float angle);
...
inline void Matrix4::column(unsigned int column, const Vector4f& value)
{
	assert(column < 4);
...
inline const Vector4f& Matrix4::column(unsigned int column) const
{
	assert(column < 4);
...
inline void Matrix4::row(unsigned int row, const Vector4f& value)
{
	assert(row < 4);
...
inline Vector4f Matrix4::row(unsigned int row) const
{
	assert(row < 4);

	return Vector4f(_m[0].element(row), _m[1].element(row), _m[2].element(row), _m[3].element(row));
}

inline void Matrix4::Transform(Vector4f& inout) const
{
	inout = Vector4f(row(0).dot(inout),
				   row(1).dot(inout),
				   row(2).dot(inout),
				   row(3).dot(inout));
}
...
{
	out = Matrix4(Vector4f(1, 0, 0, 0), Vector4f(0, 1, 0, 0),
				  Vector4f(0, 0, 1, 0), Vector4f(0, 0, 0, 1));
}

inline void Matrix4::Translation(Matrix4& out, const Vector3f& translation)
{
	out = Matrix4(Vector4f(1, 0, 0, 0), Vector4f(0, 1, 0, 0),
				  Vector4f(0, 0, 1, 0), Vector4f(translation, 1));
}
...
inline void Matrix4::PerspectiveFov(Matrix4& out, float fov, float aspect, float nearclip, float farclip)
{
	...
	out = Matrix4(Vector4f(f / aspect, 0, 0, 0),
				  Vector4f(0, f, 0, 0),
				  Vector4f(0, 0, (farclip + nearclip) / (nearclip - farclip), -1),
				  Vector4f(0, 0, (2 * farclip * nearclip) / (nearclip - farclip), 0));
}

inline void Matrix4::LookAt(Matrix4& out, const Vector3f& eye, const Vector3f& at, const Vector3f& up)
{
	Vector3f upnorm = up.unit();
	Vector3f forward = (at - eye).normalize();
	Vector3f side = forward.cross(upnorm);
	upnorm = side.cross(forward);

	Matrix4 translate;
	Translation(translate, -eye);

	out = Matrix4(Vector4f(side.x(), upnorm.x(), -forward.x(), 0),
					Vector4f(side.y(), upnorm.y(), -forward.y(), 0),
					Vector4f(side.z(), upnorm.z(), -forward.z(), 0),
					Vector4f(0, 0, 0, 1)) * translate;
}

inline void Matrix4::Scale(Matrix4& out, float scale)
{
	out = Matrix4(Vector4f(scale, 0, 0, 0),
				Vector4f(0, scale, 0, 0),
				Vector4f(0, 0, scale, 0),
				Vector4f(0, 0, 0, 1));
}

inline void Matrix4::Scale(Matrix4& out, const Vector3f& scale)
{
	out = Matrix4(Vector4f(scale.x(), 0, 0, 0),
				Vector4f(0, scale.y(), 0, 0),
				Vector4f(0, 0, scale.z(), 0),
				Vector4f(0, 0, 0, 1));
}

inline void Matrix4::RotationAxis(Matrix4& out, const Vector3f& axis, float angle)
{
...
inline void Matrix4::Ortho2D(Matrix4& out, float left, float right, float top, float bottom)
{
	...
	out = Matrix4(Vector4f(2 / rightMinusLeft, 0, 0, 0),
				  Vector4f(0, 2 / topMinusBottom, 0, 0),
				  Vector4f(0, 0, 1, 0),
				  Vector4f(-(right + left) / rightMinusLeft, - (top + bottom) / topMinusBottom, 0, 1));
}
